...
def mask_image(image, face_location, configuration):
    ...
    if angle < -threshold:
        ...
        #     plt.scatter(point[0], point[1], label=i, lw=3)
        # plt.legend()
        # plt.show()
        six_points[0][0] *= 0.98 - delta
        six_points[1][1] *= 1.01 + delta
        six_points[2][0] *= 1.02 + delta
        six_points[3][0] *= 1.02 + delta
        six_points[3][1] *= 1.02 + delta
        six_points[4][1] *= 1.08 + delta
        six_points[5][0] *= 0.98 - delta
        six_points[5][1] *= 1.02 + delta

    cfg = read_cfg(
        config_filename="masks/masks.cfg", mask_type=mask_type, verbose=False
    )
    ...
    if configuration.get("mask_patter"):
        ...
        img = texture_the_mask(
            img,
            configuration.get("mask_patter"),
            configuration.get("mask_pattern_weight"),
        )
    ...
    if configuration.get("mask_color"):
        ...
        img = color_the_mask(
            img,
            configuration.get("mask_color"),
            configuration.get("mask_color_weight"),
        )
    ...
    dst_mask = cv2.warpPerspective(
        img, M, (image.size[0], image.size[1]), flags=cv2.INTER_CUBIC
    )
    ...
    if configuration.get("mask_filter_output"):
        masked_face = masked_face.filter(
            ImageFilter.GaussianBlur(
                radius=configuration.get("mask_filter_radius")
            )
        )
    return masked_face
...
def color_the_mask(mask_image, color, intensity):
    ...
    mask_color = cv2.addWeighted(
        mask_image, 1 - intensity, color_image, intensity, 0
    )
