...
def get_face_keypoints_detecting_function(minimal_confidence: float = 0.95):
    ...
    def get_keypoints(image: Image) -> Optional[Dict]:
        ...
        results = [
            ...
            if item["confidence"] > minimal_confidence
        ]
...
def plot_face_detection(
    image: Image, ax, face_keypoints: Optional, hyp_ratio: float = 1 / 3
):
...
def get_crop_points(
    image: Image, face_keypoints: Optional, hyp_ratio: float = 1 / 3
) -> Image:
...
def crop_face(
    ...
    left, upper, right, lower = get_crop_points(
        image, face_keypoints, hyp_ratio
    )
