...
from utils import download_data, image_to_array, load_image
from utils.face_detection import (
    crop_face,
    get_face_keypoints_detecting_function,
)
...
class DataGenerator:
    def __init__(self, configuration):
        ...
        self.face_keypoints_detecting_fun = (
            get_face_keypoints_detecting_function(self.minimal_confidence)
        )
    ...
    def generate_images(
        self, image_size=None, test_image_count=None, train_image_count=None
    ):
    ...
    def generate_data(self, number_of_images, image_size=None, save_to=None):
        ...
        if save_to is valid path to folder images are saved there otherwise
        generated data are just returned in list
        """
        inputs = []
        outputs = []
        ...
        for i, file in tqdm(
            enumerate(random.sample(self.get_files_faces(), number_of_images)),
            ...
            # Generate mask
            image_with_mask = mask_image(
                copy.deepcopy(image), face_landmarks, self.configuration
            )
            ...
            if save_to is None:
                ...
                res_original.save(
                    os.path.join(save_to, "outputs", f"{i:06d}.png")
                )
    ...
    def get_dataset_examples(self, n=10, test_dataset=False):
        ...
        is not provided or False it will return images from training
        part of dataset.
        If `test_dataset` parameter is True it will return images from testing
        part of dataset.
        """
        if test_dataset:
